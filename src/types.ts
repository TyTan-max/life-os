...
export interface Settings {
  ...
  // Merchants dismissed from the "Suggested from your transactions" subscription list, keyed by
  // normalized merchant name -> the detected occurrence count at the moment of dismissal. The
  // detector re-derives suggestions from transaction history on every render with no memory of
  // its own, so without this a dismissed false positive (e.g. a recurring restaurant charge)
  // would reappear every time the page is viewed. Storing the count (rather than just a boolean)
